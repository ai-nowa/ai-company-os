...
def needs_executive_model(text: str) -> bool:
    return _contains_any(text, decision_keywords()) or _contains_any(text, crisis_keywords())
...
def resolve_model_route(
    ...
    if backend == "codex":
        ...
        return ModelRoute(
            ...
            escalated=tier in {"crisis", "executive"} or EFFORT_RANK.get(effort, 0) >= EFFORT_RANK["high"],
        )
    ...
    return ModelRoute(
        ...
        max_output_tokens=_mode_output_cap(mode),
        escalated=tier in {"crisis", "executive"} or _claude_family(model) == "opus",
    )
